...
export default function CelebrityProfilePage() {
  ...
  useEffect(() => {
    const fetchCelebrity = async () => {
      try {
        const celebRes = await axios.get(
          `http://localhost:5000/api/users/celebrities?userId=${id}`
        );
  ...
  const fetchMorePosts = useCallback(async () => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:5000/api/posts/user/${id}?page=${page}&limit=5`
      );
        ...
        <div className="flex items-center gap-4 mb-6 p-4 bg-white rounded-xl shadow-sm border border-gray-100">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              @Celebrity
              {celebrity.username ||
                celebrity.name.toLowerCase().replace(/\s/g, "")}
            </p>
          </div>
        </div>
      ) : (
      ...
      <InfiniteScroll
        ...
        endMessage={
          <div className="text-center py-8 text-gray-500">
            You've reached the end of {celebrity?.name || "this celebrity"}'s
            posts
          </div>
        }
      >
              ...
              {post.showComments && (
                <div className="px-4 py-3 border-t border-gray-100 bg-gray-50">
                  ...
                  <div className="flex space-x-2">
                    ...
                    <button
                      onClick={() =>
                        handleAddComment(post._id, post.newComment)
                      }
